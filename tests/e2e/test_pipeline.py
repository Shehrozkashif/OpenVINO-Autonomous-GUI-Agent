...
Run: python tests/e2e/test_pipeline.py
"""
import os
import sys
import time
...
try:
    ...
    print("  [OK] Input backend: Win32 SendInput (ctypes)")
    results["controller"] = PASS
except Exception as e:
    print(f"  [FAIL] Controller init failed: {e}")
...
#   Option A: key_press winleft  (Start menu search launcher)
#   Option B: hotkey ctrl+alt+t  (direct shortcut)
uses_launcher = any(s.action_type == "key_press" and s.key == "winleft" for s in steps)
...
print("  Executing: key_press 'winleft'  (opens Start menu search)")
step_super = ActionStep(id=1, subtask_id=1, action_type="key_press", key="winleft",
                        target=None, value=None, description="open launcher", verification="")
ok1 = actor.execute(step_super)
time.sleep(1.2)

print("  Executing: type 'Windows Terminal'")
step_type = ActionStep(id=2, subtask_id=1, action_type="type", value="Windows Terminal",
